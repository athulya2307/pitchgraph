...
const parsePitchData = (text) => {
  ...
  for (const line of lines) {
    ...
    const t = parseFloat(tokens[0]); // Use raw time
    const p = parseFloat(tokens[1]);
    if (isNaN(t) || isNaN(p)) continue;
    data.push({ x: t, y: p }); // Use raw time and pitch as x and y
  }
...
const PitchGraphCard = ({
  ...
  const drawGraph = () => {
    ...
    // If no data or invalid range, show message and return
    if (
      !filteredData.length ||
      !startTime ||
      !endTime ||
    ...
    // Prevent division by zero if all pitches are the same
...
const PitchGraphMulti = () => {
  ...
  const [graphType, setGraphType] = useState('victory'); // State to track selected graph type
...
const styles = {
  ...
  title: {
    fontSize: 56,
    fontWeight: 800,
    color: '#1e40af', // Indigo color for title
    marginBottom: 48,
    userSelect: 'none',
    textAlign: 'center',
  ...
  select: {
    ...
    outlineOffset: 2,
    color: '#1e40af', // Indigo color for select
  },
  grid: {
    display: 'grid',
  ...
  cardTitle: {
    fontWeight: 700,
    fontSize: 24,
    color: '#1e40af', // Indigo color for card title
    marginBottom: 24,
    userSelect: 'text',
  ...
  fileLabel: {
    fontWeight: 600,
    fontSize: 15,
    color: '#2563eb', // Indigo-600 for file label
    cursor: 'pointer',
    userSelect: 'none',
    marginRight: 12,
  ...
  miniLabel: {
    fontSize: 14,
    fontWeight: 600,
    color: '#1e3a8a', // Indigo-900 for mini labels
    marginBottom: 4,
    userSelect: 'none',
  },
  miniInput: {
    ...
    border: '1.5px solid rgba(37, 99, 235, 0.7)',
    outlineOffset: 2,
    color: '#1e40af', // Indigo-800 for input text
    width: 110,
  },
  fileName: {
    fontSize: 13,
    fontStyle: 'italic',
    color: '#2563eb', // Indigo-600 for file name
    marginTop: 4,
    whiteSpace: 'nowrap',
    overflow: 'hidden',
  ...
  chartWrapper: {
    ...
    overflow: 'hidden',
    height: 280,
    border: '1.5px solid #3b82f6', // Indigo-500 for chart border
